...
// TODO: Move all magic strings to top
...
function ThousandsBusinessLogic(number){
  ...
  if(number < 2000) return `mille-${numbersToFrenchWords(number % 100)}`;
...
readLine.question('Enter a comma seperated list: ', (input) => {
    if(!IsValid(input)){
      console.error('Error: comma seperated list not provided or invalid'); 
    ...
    console.log("Numbers to French words: ", result.join(', '));
    readLine.close();
  });
